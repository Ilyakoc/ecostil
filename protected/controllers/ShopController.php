...
class ShopController extends Controller
{
    ...
    public function actionAddproduct(){
        ...
        $sql = "INSERT INTO `with_product`(`product_id`, `product_add`) VALUES ($object[1] , $object[0])";
        $connection=Yii::app()->db; // так можно делать, если в конфигурации настроен компонент соединения "db"
        $command=$connection->createCommand($sql);
        $rowCount=$command->execute();
        $this->renderJSON(['count' => $rowCount]);
    }
    ...
    public function actionSearchproduct(){
        ...
        $criteria->compare('title', $object);

        $products = Product::model()->findAll($criteria);

        if ($products) {
            $this->renderPartial('/site/block/products', [
                'models' => $products,
            ]);
        } else {
            echo '<p>Нет записей</p>';
        }
        //$this->renderJSON(['count' => $products]);
    }
    ...
    public function actionRemoveproduct(){
        ...
        $object = json_decode($input, true);
        $object = explode('-', $object);
        $sql = "DELETE FROM `with_product` WHERE `product_id` = $object[1] AND`product_add` = $object[0]";
        $connection=Yii::app()->db; // так можно делать, если в конфигурации настроен компонент соединения "db"
        $command=$connection->createCommand($sql);
        $rowCount=$command->execute();
        $this->renderJSON(['count' => $rowCount]);
