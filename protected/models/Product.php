...
//         	'filesBehavior'=>'\file\behaviors\FilesBehavior',
    ...
    public function eav($multi=false)
    {
        ...
        print_r('111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111111');
        die();
        if(isset($data_json)){
            ...
            if(count($attr_filter)>0){
                ...
                foreach ($attr_filter as $key => $attr) {
                    ...
                    //if($attr->value='on') $attr->value = 1;
                    $values[]=$attr->value;
                    $valueKey=(int)preg_replace('/^[^0-9]+$/', '', $key);
                 ...
                 //die();
                if(!empty($values)) {
                    $eavValues=HDb::queryAll('SELECT `id_attrs`, `value` FROM `eav_value` WHERE `value` IN  (' . "'". implode("', '", $values) .  "'".')');
                    ...
                    if(!empty($eavValues)) {
                        ...
                        foreach($eavValues as $eavItem) {
                            if($attribute != array($eavItem['id_attrs'],$eavItem['value'])){
                                $attribute =array($eavItem['id_attrs'],$eavItem['value']);
                                $attributes[]=$attribute;
                                $i[]=$eavItem['id_attrs'];
                            }
                        }

                        //print_r(count(array_unique($i)));die();
                        foreach ($attributes as $key){
                            $subquery[]= "(id_attrs={$key[0]} AND value IN ('{$key[1]}'))";
                        }
                        $subqueries[]= implode(' OR ', $subquery);


                        $query.=implode(' OR ', $subqueries);
                        $query.=' GROUP BY id_product HAVING count(id_product)='.count(array_unique($i));
