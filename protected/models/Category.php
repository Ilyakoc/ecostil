...
class Category extends \common\components\base\ActiveRecord
{
    ...
    public function getMax($categories, $related, $column)
    {
        $categories = implode(', ', $categories);

        if ($related != NULL)  {
            $related = implode(', ' ,array_keys($related));
            $query = "SELECT MAX(`$column`) FROM `product` WHERE `category_id` IN ({$categories}) OR `id` IN ({$related})";
        } else {
            $query = "SELECT MAX(`$column`) FROM `product` WHERE `category_id` IN ({$categories})";
        }

        return (int)HDb::queryScalar($query);
    }
     ...
     * @return number|false возвращает false, если идентификатор не задан.
     */
    public function getMin($categories, $related, $column)
    {
        $categories = implode(', ', $categories);

        if ($related != NULL)  {
            $related = implode(', ' ,array_keys($related));
            $query = "SELECT MIN(`$column`) FROM `product` WHERE `category_id` IN ({$categories}) OR `id` IN ({$related})";
        } else {
            $query = "SELECT MIN(`$column`) FROM `product` WHERE `category_id` IN ({$categories})";
        }
